...
function showSeatDetails(pos) {
  ...
  avaliableSeats = countNeighbors(gCinema, pos.i, pos.j);
  txtHtml = createTxtHtml(avaliableSeats);
  elPopup.querySelector("h4 span").innerText = `${txtHtml}`;
...
function countNeighbors(mat, cellI, cellJ) {
  ...
  for (var i = cellI - 1; i <= cellI + 1; i++) {
    ...
    for (var j = cellJ - 1; j <= cellJ + 1; j++) {
      ...
      if (!mat[i][j].isBooked && mat[i][j].type === 'S') neighbors.push({ i: i, j: j });
    }
  }
  return neighbors;
}

function createTxtHtml(avaliableSeats){
    var txtHtml = `\n`;
    for(var i =0;i < avaliableSeats.length;i++){
        var avaliableSeat = avaliableSeats[i];
        txtHtml += `${avaliableSeat.i}-${avaliableSeat.j}\n`;
      }
      return txtHtml;
}